...
export default function App() {
    ...
    return (_jsxs("div", { className: "safe-top mx-auto min-h-full max-w-md px-4 pb-10", children: [_jsxs("header", { className: "flex items-center justify-between py-4", children: [_jsxs("h1", { className: "text-xl font-bold tracking-tight", children: ["My", _jsx("span", { className: "text-accent", children: "Gym" })] }), _jsxs("div", { className: "flex items-center gap-3", children: [pending > 0 && (_jsxs("button", { onClick: () => void syncNow(), className: "rounded-full bg-surface2 px-2 py-1 text-xs text-slate-400", title: "Tap to sync now", children: ["\u2601 ", pending, " unsynced"] })), _jsx("button", { onClick: () => setProgramOpen(true), "aria-label": "Program settings", className: "text-xl text-slate-400", children: "\u2699" })] })] }), view === 'home' ? (_jsxs("div", { className: "space-y-4", children: [_jsx(InstallHint, {}), _jsx(Hero, { program: program, rec: rec, onStart: start }), _jsx(ConsistencyChips, { log: log, today: today }), _jsx(StatTiles, { log: log }), _jsx("button", { onClick: () => setMatchOpen(true), className: "w-full rounded-xl border border-line bg-surface py-3 font-medium text-slate-200", children: "\u26BD I played \u2014 log a match" })] })) : (_jsx(WorkoutView, { program: program, session: activeSession, cType: cType, slot3Next: slot3Next, legAppend: legAppend, onPlay: (url, title) => setVideo({ url, title }), onBack: () => setView('home'), onFinish: () => setFinishOpen(true) })), video && _jsx(VideoModal, { url: video.url, title: video.title, onClose: () => setVideo(null) }), _jsx(LogWorkoutSheet, { open: finishOpen, program: program, session: activeSession, defaultCType: cType, defaultSlot3: slot3Next, defaultLegAppend: legAppend, onClose: () => setFinishOpen(false), onSubmit: (e) => {
                    addGym(e);
                    void syncNow();
                    setView('home');
                    window.scrollTo(0, 0);
                } }), _jsx(LogMatchSheet, { open: matchOpen, onClose: () => setMatchOpen(false), onSubmit: (e) => {
                    addMatch(e);
                    void syncNow();
                } }), _jsx(ProgramSheet, { open: programOpen, onClose: () => setProgramOpen(false) })] }));
}
