...
export default function App() {
    ...
    return (_jsxs("div", { className: "safe-top mx-auto min-h-full max-w-md px-4 pb-10", children: [_jsxs("header", { className: "flex items-center justify-between py-4", children: [_jsxs("h1", { className: "text-xl font-bold tracking-tight", children: ["My", _jsx("span", { className: "text-accent", children: "Gym" })] }), _jsx("button", { onClick: () => setProgramOpen(true), "aria-label": "Program settings", className: "text-xl text-slate-400", children: "\u2699" })] }), view === 'home' ? (_jsxs("div", { className: "space-y-4", children: [_jsx(Hero, { program: program, rec: rec, onStart: start }), _jsx(ConsistencyChips, { log: log, today: today }), _jsx(StatTiles, { log: log }), _jsx("button", { onClick: () => setMatchOpen(true), className: "w-full rounded-xl border border-line bg-surface py-3 font-medium text-slate-200", children: "\u26BD I played \u2014 log a match" })] })) : (_jsx(WorkoutView, { program: program, session: activeSession, cType: cType, slot3Next: slot3Next, legAppend: legAppend, onPlay: (url, title) => setVideo({ url, title }), onBack: () => setView('home'), onFinish: () => setFinishOpen(true) })), video && _jsx(VideoModal, { url: video.url, title: video.title, onClose: () => setVideo(null) }), _jsx(LogWorkoutSheet, { open: finishOpen, program: program, session: activeSession, defaultCType: cType, defaultSlot3: slot3Next, defaultLegAppend: legAppend, onClose: () => setFinishOpen(false), onSubmit: (e) => {
                    addGym(e);
                    setView('home');
                    window.scrollTo(0, 0);
